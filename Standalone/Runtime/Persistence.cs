...
namespace TapTap.Achievement.Standalone 
{
    ...
    internal class Persistence 
    {
        ...
        internal Persistence(string dir, string fileName) 
        {
            string newDirecotryName = TapAchievement.ACHIEVEMENT_DIR;
            if (!string.IsNullOrEmpty(LCCore.AppId)) {
                newDirecotryName = newDirecotryName + "_" + LCCore.AppId;
            }
            string newDirectoryPath = Path.Combine(Application.persistentDataPath, newDirecotryName);
            // 兼容旧版本数据
            if(!Directory.Exists(newDirectoryPath)){
                string oldDirectoryPath = Path.Combine(Application.persistentDataPath, TapAchievement.ACHIEVEMENT_DIR);
                if (Directory.Exists(oldDirectoryPath)) {
                    Directory.Move(oldDirectoryPath, newDirectoryPath);
                }
            }
            _filePath = Path.Combine(Application.persistentDataPath, newDirecotryName, dir, fileName);
        }
